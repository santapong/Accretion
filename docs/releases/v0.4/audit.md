...
> **Completed historical release audit.** The v0.4.0 tables record measurements
> on the audited commit on 2026-09-07; the v0.4.1 addendum names its own base and
> checks. Both released tags are pinned in the [frozen baseline](baseline.md).
> Status and limitation wording was reconciled on 2026-09-08; this did not rerun
> the tests, the locked analysis or live providers.

> Audit date: 2026-09-07 (Asia/Bangkok)
>
> Release-finalization base: `develop` at `cea73eb1eeb6e1cdeb7513de5acff3b085c16542` (the #156 squash commit that closed M10); release-finalization PR #157 added versions, lockfiles and the release documents on top of it
>
...
| Release-finalization package metadata | PR #157 set `0.4.0` in `pyproject.toml`, the root and UI `package.json`, and `accretion.__version__`; the preceding milestone code base did not bump versions |
...
The audited work was promoted to `main` through the protected bridge in
[branch-policy.md](../../governance/branch-policy.md), then tagged. The completed
procedure and pinned identities below are the release evidence; current branch
heads may contain later work.
...
**Historical measurement on 2026-09-07.** 167 = the 117 criteria of the v0.1–v0.3 SDDs plus the fifty
...
`AC4-M10-045`..`-050`. The released audit confirmed `167 / 159 / 0`;
subsequent checks must record their actual outcomes rather than inherit this measurement.
...
| `TestSetAccessLog` row count at the v0.4.0 audit | **2**, one read per corpus. The v0.4.1 amendment added two separately authorized rows, for **4 total** in [the current access log](../../research/v0.4/access-log.jsonl). The original measurement and its scope remain unchanged |
...
| Original v0.4.0 safety gates | **Both fail for every policy on both corpora** under the original conservative pooling. ADR4-M10-005 and the original tables remain unchanged; the separately frozen amendment-1 analysis is reported in the v0.4.1 addendum |
...
3. **Reservation replay exists; settlement recovery needs a bounded check.**
   Clarified 2026-09-08: `LedgerRegistry` reconstructs readable EXPLORE reservation
   charges from immutable receipts. Settlement adjustments are volatile and can
   exceed the reservation, so restart is not proven to retain all observed cost.
   Absolute caps use the same cumulative workspace/node-class ledger; they are
   not a separate durable monetary or per-day limit. See the
   [open follow-up work](backlog.md#remaining-bounded-work).
4. **Selected TOOL nodes enforce exact bindings; routed AGENT tool sessions
   remain denied.** Clarified 2026-09-08: `invoke_selected` uses the existing
   capability gateway with receipt-pinned binding checks. The AGENT session path
   cannot honor those pins and is refused before dispatch/session creation.
   A crash after a durable dispatch claim remains uncertain execution to be
...
5. **`exploration_policy` moved every pre-delta `ObjectiveContract` content hash.** ADR-056's
   ...
   digest-neutral. At that pre-release freeze delta, the released v0.3.0 schema
   ended at 0016 and the new objective-contract writer was exercised only by
   tests. That historical compatibility rationale does not describe the schema
   after the v0.4 releases shipped migration 0017.
6. **The generic `DRIFT` split and the M10 holdout are distinct.** The split
   allocator uses the seeded lineage-root hash; M10d supplied a separate holdout
   with later provider/runtime serving windows. The holdout does not make the
   generic splitter an era-aware algorithm. See [M10's evidence](m10-plan.md).
