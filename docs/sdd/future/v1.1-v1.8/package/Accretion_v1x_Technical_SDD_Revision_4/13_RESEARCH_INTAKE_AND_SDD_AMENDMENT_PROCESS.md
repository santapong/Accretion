...
For this authorized Revision-4 document amendment, material choices are recorded in document 28. Review status is DOCUMENT_AMENDED / IMPLEMENTATION_LOCKED; independent architecture, methods and security review remains pending and no reviewer signature is fabricated. The full eight-release design is updated directly with schemas, events, fixtures and acceptance traceability. Research plans retain numerical/owner gates until concrete data makes them resolvable.
...
Revision 4 also records effective-setting observation, mature harness baselines, stochastic no-op replay, delayed memory poisoning and serialized consumption, staged behavior screening, optional harness/adapter interaction, context-only transfer, and separately scoped workflow/scenario calibration. See `25_RESEARCH_SOURCE_REGISTER.md` for exact selectively read primary versions and source conflicts; no author-reported outcome is an Accretion result.
