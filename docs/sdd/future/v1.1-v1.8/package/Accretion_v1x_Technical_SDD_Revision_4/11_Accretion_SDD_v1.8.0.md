...
Keep two independent protocols. The new research payloads and gates are normative design requirements; empirical benefits remain unproven.
...
The Simulation Evaluator separates fixed-graph workflow allocation from a later scenario-prediction calibration study. A transparent deterministic allocator is the first control. Scenario correction, if studied, is an offline evidence model under the existing evaluator, not an additional controller.
...
PhysicalOptimizationAdvisory:
  ...
  predicted_outcomes:
    ...
    uncertainty_statement_ref: ArtifactRef  # scope must support the claimed bound
...
`ResearchComparisonPlan` distinguishes `WORKFLOW_ALLOCATION` and `SCENARIO_CALIBRATION`. Workflow manifests pin the graph, permitted digital choices, mandatory verifiers and simulated resources. Scenario manifests pin simulator/target identities, scene/configuration groups, paired-data budget and separate fit, selection, calibration and final roles. `UncertaintyStatement` identifies marginal prediction coverage explicitly.
...
For allocation, compare FCFS, static critical-path priority, a bounded deterministic constrained solver and the learned advisory using the same finite candidate graph and resource model. Vary latency estimates and queue uncertainty in frozen sensitivity scenarios. For scenario calibration, fit simulator correction/predictor on fit/development data, freeze selection, calibrate on untouched groups and evaluate on a final group-disjoint set. An out-of-range conformal rank produces the registered conservative full-support set, never a clamped maximum.
...
Store separate workflow and scenario study identities, immutable scene/simulator/target pair manifests, data-role access receipts and prediction/calibration versions. Every changed predictor or score requires requalification and fresh eligible calibration; physical outcomes retain their exact trial lineage.
...
Infeasible or uncertain allocation returns the static admissible fallback. Missing target pairs or unusably broad calibration yields simulation-only or INCONCLUSIVE scope and abstention; it does not narrow intervals to force an advisory or create physical authorization.
...
Label workflow simulation, sim-to-sim prediction and exact-target physical evidence separately. Show deterministic comparator, queue sensitivity, calibration unit/size, interval kind/usefulness and abstention; a marginal interval is not a safety approval.
...
- automatic physical retry dispatch is impossible; any observed prohibited request or re-execution remains truthfully recorded and handled as required;
...
Test deterministic-solver infeasibility and timeout, changed mandatory verifier, post-freeze mutation, latency/queue misspecification, selection/calibration overlap, scene leakage, undersized calibration, out-of-scope target data and sim-to-physical relabeling. Preserve independent freeze/approval/safety tests.
...
### 19.1 Revision-4 workflow advisory boundary
...
Keep two independent protocols. Allocation is command-disabled and estimates digital simulated workflow burden only. Scenario calibration begins sim-to-sim, comparing simulator-only, target-only, simple residual correction and a scenario-conditioned model at the same paired-data budget. Physical applicability additionally requires exact-task target physical pairs and the existing v0.6 process. Split by independent scene/configuration when that generalization is claimed. Report prediction error, interval usefulness, task success, abstention and physical safety separately; marginal intervals cannot certify each scenario as safe.
...
| P4 | Shadow/simulation evaluator | Separate workflow/scenario protocols, deterministic controls and limitation report |
...
- [ ] **AC18-P4-021** Command-disabled workflow allocation includes a bounded deterministic solver and queue/latency sensitivity on the same frozen graph, resources and mandatory verification.
- [ ] **AC18-P4-022** If scenario calibration is launched, simulator-only, target-only, residual-correction and scenario-model controls share paired-data budgets and independent selection, calibration and scene/configuration test roles.
- [ ] **AC18-P3-023** Scenario intervals retain their marginal scope, conservative small-sample handling and out-of-scope abstention; simulation or prediction coverage never establishes per-scenario physical safety.
...
| P-OQ11 | Scenario-calibration readiness | Separate later sim-to-sim study; exact-target physical pairs remain an independently gated requirement | P3/P4 protocol freeze |
...
Any later release inherits distinct workflow, scenario and physical evidence classes and cannot turn a calibration artifact into control or approval authority.
