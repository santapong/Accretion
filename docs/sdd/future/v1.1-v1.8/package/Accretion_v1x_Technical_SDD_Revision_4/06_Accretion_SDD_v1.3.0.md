...
Report localization metrics and abstention separately from independent repair PASS and saved descendant work. The new research payloads and gates are normative design requirements; empirical benefits remain unproven.
...
The Failure Ownership Resolver records localization uncertainty and multi-cause ambiguity. Recovery research separates diagnosis, evidence from actual intervention, and cost-effective recovery. The Replay Service is the existing recovery research path; a learned attribution never changes failure ownership or supplies its own acceptance.
...
`ObservableInterventionRecord` schema 2 records localization status, actual replay and no-op comparison counts, and a paired comparison reference. `OBSERVED_REEXECUTION` means actual execution, not deterministic LLM output. `ResearchComparisonPlan` kind `RECOVERY_REPLAY` binds full retry, no-op restore/replay, rule-guided intervention and proposed intervention arms.
...
Freeze intervention selection on development evidence. Restore each held-out eligible episode, then compare predeclared matched repetitions of full retry, no-op replay, rule-guided and proposed interventions. Isolate one changed component and record whether random seeds are honored; identical seeds do not make hosted calls deterministic. A bounded causal claim requires the comparison and uncertainty analysis, not merely one successful replay.
...
Store prediction, localization, intervention and comparison references separately in the attempt-chain lineage. Each repeat is a new immutable execution; evidence from a verbal prediction is never upgraded into an observed replay.
...
Failed restoration, unverifiable side effects or absent independent verification prevents a causal conclusion and any replay dispatch not already allowed by the existing recovery policy. Retain diagnostic-only output or close the study INCONCLUSIVE/DEFERRED.
...
Separate diagnostic localization, observed intervention result and net recovery value. Show no-op/full-retry outcomes, repeat uncertainty, restoration coverage and complete overhead.
...
Exercise multi-cause ambiguity, predicted-only validation, a lucky single replay, absent no-op arm, unhonored seeds, contaminated gold access, incomplete restoration and side-effect failure. Compare the proposed repair against ordinary stochastic reruns before attributing a benefit.
...
### 19.1 Revision-4 observable-intervention replay study
...
Report localization metrics and abstention separately from independent repair PASS and saved descendant work. Preserve ambiguous multi-cause episodes in their declared population. Charge instrumentation, restore, invalidation, repeated execution and verification to recovery. A diagnostic ranking may remain useful when restoration is unavailable, but then the causal/recovery claim is UNSUPPORTED.
...
| R8 | Scientific and operational gate | Matched replay/no-op report, uncertainty, full cost and security sign-off |
...
- [ ] **AC13-R8-018** Recovery research separates uncertain localization, observed intervention and net recovery value using frozen full-retry, no-op, rule-guided and proposed arms with predeclared repeated comparisons.
- [ ] **AC13-R1-019** Observed re-execution is distinguished from deterministic output, and absent restoration, no-op comparison or independent evidence cannot support a bounded causal claim.
...
| R-OQ7 | Replay repetitions and ambiguity | Predeclare supported matched repeats and ambiguous-label handling; diagnostic-only where restore is unavailable | R3/R8 protocol freeze |
...
The state-coordination handoff distinguishes observed state-related failures from diagnostic attribution and retains replay cost and restoration limits.
