# Accretion v1.x Research Source Register — Revision 4
...
The WHALE test-selection statement and SCAPE appendix details were checked against downloaded versioned arXiv PDFs, not only alphaXiv's extracted pages. The primary PDF SHA-256 values are SCAPE `e4a64b7246feaf98576c06ef24a3093f649b118007eee54e9a6efa5207cffebc` and WHALE `f4307846da7686ee75ef070fdc53112c8eb8ca557b9318da0a88838e56b9e602`. Official code identities are in `research/code-manifest.json`. Full downloaded source files remain in the original local research workspace and are excluded from this package.
...
## Earlier sources and current design use

The exact earlier source-review record is preserved in `history/revision-3/25_RESEARCH_SOURCE_REGISTER.md`. Its selectively read Task-CoEvolve, AgentRewardBench, Conformal Thinking, Memory Poisoning and HexAGenT constraints remain inherited evidence with the original access date and scope; they were not re-read for this document integration. Who & When and AgentDojo now have the selective primary review above. CausalFlow's description conflict is retained. GraphTracer and other synthesis-only leads do not support a new mandatory claim.

All twelve current source cards support design mechanisms or limits, not transferred performance promises. The owning SDDs apply those mechanisms as normative evaluation/design requirements; the claimed Accretion benefit remains PROPOSED until measured. Document 28 maps each source to its specific release changes and rejected portions.
