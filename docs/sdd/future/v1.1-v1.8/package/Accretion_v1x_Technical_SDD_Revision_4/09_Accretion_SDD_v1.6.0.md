...
The optional four-arm study uses one total research budget including proposer, rollouts, failed candidates, training, evaluation and serving. The new research payloads and gates are normative design requirements; empirical benefits remain unproven.
...
The optional lab may study a bounded interaction between harness search and adapter training. Harness proposals remain with v0.10/v1.5; weight updates remain with v1.6. The assembled base-model/harness/adapter pair is requalified after either component changes.
...
`ResearchComparisonPlan` kind `JOINT_ADAPTATION` binds four arms: fixed baseline, harness-only, adapter-only and alternating. Its hypothesis manifest pins base model, harness, adapter, rollout-generating pair, training data and phase schedule. Selection role is DEVELOPMENT; test-based checkpoint selection is invalid.
...
Start with verified-success SFT on one eligible repeatable family and a small supported model. Choose a short phase schedule using development evidence, then freeze the assembled pair. Generate training records with the exact rollout harness/adapter identity. Recheck joint compatibility and verifier qualification after each change. Do not assume a one-way optimization order is superior without this optional comparison.
...
Link phase schedules, generated data and evaluation to both harness and adapter digests. Each changed pair creates a new candidate identity; old test results cannot qualify the new pair. Store failed phases and cost alongside successful candidates.
...
A pair-identity or qualification mismatch blocks training/evaluation progression. A budget or amortization failure preserves the frozen base configuration and closes the optional study under the existing governed negative/deferred process.
...
Compare all four arms with total resource use and break-even sensitivity. Display development nomination, final lock, joint requalification and the distinction between lab readiness, study closure and promotion.
...
Test selection by final accuracy, mixed rollout/training pair identity, unqualified changed harness, missing proposer/failed-training cost, incompatible assembled pair and negative/deferred closure attempting promotion. These tests do not require training a model.
...
### 19.1 Revision-4 label and calibration qualification
...
Adaptive rollout allocation may use uncertainty or intermediate scores only when the score is defined, logged and available at the same point in every arm. Calibration is evaluated time-forward and by task/project shift. A passed exchangeability diagnostic cannot establish a theorem assumption. Any formal coverage or conformal guarantee requires the shared typed assumption and label-timing record and is withdrawn when an applicable assumption fails or filtering invalidates its scope; the system falls back to a fixed conservative rollout cap and independent verification.
...
The optional four-arm study uses one total research budget including proposer, rollouts, failed candidates, training, evaluation and serving. Component-matched update counts alone are insufficient. Report one-off cost and realistic break-even volumes; terminate or defer when recurrence cannot repay the experiment. No test-selected pair or paper-scale hardware allocation is a default.
...
| A5 | Independent evaluation/baselines | Independent hidden report and optional four-arm joint-budget comparison |
...
- [ ] **AC16-A5-018** If the interaction study is launched, fixed, harness-only, adapter-only and alternating arms use a total budget and development-only nomination of an immutable assembled pair.
- [ ] **AC16-A2-019** Every adaptation phase binds the generating harness/adapter pair to data and requalifies the assembled candidate after either component changes; negative or deferred closure promotes nothing.
...
| A-OQ8 | Alternating study | Optional small four-arm SFT-first comparison after a demonstrated gap; freeze total cost and phase schedule | Separate lab protocol |
...
v1.7 may proceed after governed negative/deferred lab closure as already specified; it consumes context or adapter evidence only within the existing compatibility and ownership rules.
