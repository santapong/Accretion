...
For Stage B, report fix/regression transitions and uncertainty, including unsuccessful patches and broad hooks that damage unrelated tasks. The new research payloads and gates are normative design requirements; empirical benefits remain unproven.
...
After the frozen-candidate acquisition study closes, the Candidate Screener may run a separately registered behavior-linked development screen. The proposer names the target behavior, source traces, changed component and plausible collateral damage. The independent evaluator owns confirmation and final holdout; v1.4 still owns reusable experience.
...
`ResearchComparisonPlan` distinguishes `TRACE_ACQUISITION` from `BEHAVIOR_SCREENING`. The latter binds a hypothesis manifest, repair/preservation/boundary sets, fresh development confirmation, proposal history and final evaluation roles. A `HarnessComparisonReport` partitions paired outcomes into fixed, still failing, preserved, regressed and mixed/uncertain without promoting inferred attribution.
...
Stage A keeps candidate generation frozen and changes trace acquisition only. Stage B requires its own protocol after Stage A closes; compare score-only, frozen-diversity and behavior-linked screening under one total budget. A selected behavior batch can reject or refine a hypothesis but cannot estimate population improvement. Freeze the finalist after fresh development confirmation and before statistical calibration or final testing.
...
Keep typed hypothesis, candidate, paired outcome, rejected lesson and confirmation references in the existing optimization history. No parallel learning-memory service is introduced. Eligible reuse remains subject to v1.4 validation and quarantine.
...
A patch lacking source-linked behavior or preservation evidence fails the screen. A contaminated confirmation/final set blocks the claim and requires a new untouched cohort; a negative result cannot trigger further search on final-test feedback.
...
Show fixed/still-failing/preserved/regressed/mixed outcomes, selected-batch scope, fresh confirmation status and the separate final estimate. Display full search cost and non-promoted rejected lessons.
...
Test a repair that causes regression, a candidate with only favorable selected cases, missing preservation/boundary tests, reused confirmation tasks, hidden-test feedback in proposal history and a rejected lesson incorrectly promoted to Experience. Include screen false negatives and all rejected-candidate costs.
...
### 19.1 Revision-4 adaptive trace acquisition study
...
For Stage B, report fix/regression transitions and uncertainty, including unsuccessful patches and broad hooks that damage unrelated tasks. Evaluate the finalist against the mature baseline on untouched project lineages. Any iterative candidate generation must be declared as development search and cannot consume final test feedback. Preserve Stage A selection-regret claims separately.
...
| O5 | Independent held-in/hidden evaluator | Stage-separated screening, fresh-confirmation and hidden-access audit |
...
- [ ] **AC15-O5-017** Behavior-linked screening is a distinct post-acquisition protocol with source-linked hypotheses, repair, preservation, boundary and fresh development confirmation sets.
- [ ] **AC15-O8-018** Selected behavior batches cannot establish population improvement; final untouched-lineage evaluation and full fixed, preserved, regressed, uncertain and rejected-candidate accounting are retained.
...
| O-OQ8 | Behavior-screening trigger | Start only after frozen-candidate acquisition closes and a bounded patch hypothesis remains useful | Separate protocol freeze |
...
The v1.6 decision includes mature harness-only performance, remaining policy-gap evidence and a complete search-cost ledger, including failed patches.
