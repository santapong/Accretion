...
Prefer alphaXiv tools when a connected alphaXiv MCP server is present. This Codex installation does not currently expose that connector, so do not invent tool calls or block the task: search primary sources on arXiv and official paper/project repositories, state that library filing is unavailable, and continue the research. Never use secondary summaries as evidence for technical claims when a paper or official repository is available.

The existing Claude alphaXiv connector is OAuth-based and has no reusable API key. Do not
inspect, copy, or attempt to transplant its OAuth grant into Codex. If Codex later exposes an
alphaXiv connector, use its own connection flow; until then use the primary-source fallback above.
