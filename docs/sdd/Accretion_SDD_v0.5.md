...
**Status:** Active implementation baseline; M0 registration, not release acceptance\
...
Promoted on 9 September 2026 from the unchanged forward package. The
[approved M0 decisions](../releases/v0.5/m0-decisions-2026-09-09.md) resolve the
implementation profile and inherited-contract interpretation: MuJoCo with
UR5e/2F-85 and Panda, exact episode approval, prepared-command admission,
independent verifier process and identity, and versioned domain events. The
Gazebo/ROS YAML values below remain original illustrative examples; the M0
ADR governs the selected profile. This promotion preserves all original release
requirements and does not claim their implementation or experimental success.

Section 22 assigns stable source-order IDs with separate milestone owners and
required witness classes. A = automated behavior, S = actual simulator evidence,
U = frontend/production-browser evidence, B = registered scientific evidence.
All required classes are conjunctive; a parser test is not a simulator or study
result. Owners are accountable for eventual proof, not a requirement to finish
later composite evidence before their construction milestone can close. M0–M9
map in order to the ten items in §21. Intermediate deferrals remain visible;
the release gate refuses them.
...
| ID | Priority | Owner | Witnesses | Criterion |
| --- | --- | --- | --- | --- |

| AC5-001 | MUST | M0 | A | All contracts have immutable IDs, versions, canonical hashes, and schema tests. |
| AC5-002 | MUST | M2 | A | No v0.5 capability can resolve to a physical endpoint. |
| AC5-003 | MUST | M1 | A | Adapter manifests cannot grant capabilities. |
| AC5-004 | MUST | M0 | A | Unknown major contract versions fail closed. |
| AC5-005 | MUST | M4 | A | Active episodes remain pinned to admitted contract hashes. |
...
| AC5-006 | MUST | M7 | A+S | At least two adapter implementations or materially distinct versions pass conformance. |
| AC5-007 | MUST | M7 | A+S | Reset, timeout, duplicate, out-of-order, and crash tests pass. |
| AC5-008 | MUST | M2 | A+S | Simulator images and worlds use immutable digests. |
| AC5-009 | MUST | M4 | A+S | Seed and randomization capture is complete. |
| AC5-010 | MUST | M1 | A+S | Observation unit, frame, shape, and time validation is enforced. |
...
| AC5-011 | MUST | M3 | A+S | Every action intent is checked before adapter invocation. |
| AC5-012 | MUST | M3 | A+S | Joint, velocity, acceleration, workspace, and episode caps fail closed. |
| AC5-013 | MUST | M4 | A+S | Uncertain acknowledgement aborts instead of replaying a command. |
| AC5-014 | MUST | M4 | A+S | Hard caps terminate every automatic loop. |
| AC5-015 | MUST | M3 | A | Safety evaluator denial cannot be overridden by a runtime or plugin. |
...
| AC5-016 | MUST | M5 | A+S | Every accepted episode passes independent task, safety, and completeness checks. |
| AC5-017 | MUST | M5 | A+U | `INCONCLUSIVE` reaches human review when unresolved. |
| AC5-018 | MUST | M5 | A | Contradictory evidence is preserved and dependents are discoverable. |
| AC5-019 | MUST | M5 | A+S+B | Replay class is declared and verified. |
| AC5-020 | MUST | M5 | A+U | Simulation evidence cannot be exported as physical evidence. |
...
| AC5-021 | MUST | M5 | A | Only verified, complete, contradiction-free episodes become eligible experience. |
| AC5-022 | MUST | M5 | A | Different-embodiment experience is a weak prior only. |
| AC5-023 | MUST | M0 | A+B | Benchmark splits, seeds, metrics, and analyses are pre-registered. |
| AC5-024 | MUST | M9 | B | False acceptance and safety non-regression gates pass. |
| AC5-025 | MUST | M9 | A+B | Ablations isolate routing and experience effects. |
...
| AC5-026 | MUST | M8 | A+U | Experiment Studio renders backend-derived episode state. |
| AC5-027 | MUST | M4 | A | APIs enforce idempotency and optimistic concurrency. |
| AC5-028 | MUST | M4 | A+U | Event replay reconstructs episode state. |
| AC5-029 | MUST | M4 | A+S+U | Adapter and simulator failures produce actionable typed errors. |
| AC5-030 | MUST | M2 | A+S | Security tests cover adapter escape, payload limits, and endpoint confusion. |
