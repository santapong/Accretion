...
| `V02-P7-003` | Compatibility tests reject retracted, cross-repository, missing/non-ancestor, architecture/policy/verifier/capability-incompatible, max-age, and protected-side-effect evidence; the frozen corpus rejects 19/20 stale/incompatible sources. Since M8 the criterion is proven directly against `ExperienceService.assess()` across all 19 reason codes; the 19/20 figure the benchmark API serves is still corpus-declared (`stale_rejection_source: DECLARED`) — see ADR3-M8-005. |
