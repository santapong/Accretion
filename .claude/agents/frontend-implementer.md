...
`apps/ui/src/` is flat at the root with one `pages/` folder. `App.tsx` is short — `BrowserRouter`, `<OperatorShell />`, and one stylesheet import (`./theme.css`), which must stay **after** the `./OperatorShell` import because that import is what pulls React Flow's own sheet, and our unlayered overrides beside it, into the cascade ahead of the layers. `OperatorShell.tsx` is the nav bar and the router outlet; `routes.tsx` exports `RouteEntry` and `ROUTES`; each screen is a named export in `src/pages/<Name>Page.tsx` alongside its private sub-components. `RunExecution.tsx` (~980) holds the run detail with the React Flow canvas and its inspectors and `EventStream.tsx` the resumable trace — both sit at the root beside their pinned tests. `api.ts` is a hand-written fetch client; `types.ts` hand-written types; `graphLayout.ts` deterministic layout; `runState.ts`/`runDuration.ts`/`runBadges.ts`/`pages/formLines.ts` are plain `.ts` helpers, never co-located with a component; `StatePill.tsx` is one component; `api/schema.d.ts` is generated. **Styling is two files, and there is no longer a single `styles.css` rule.** M9 PR5c finished the port and deleted it. `theme.css` holds the Tailwind layers, the fonts `@import`, the tokens, and — in `@layer components` — every rule, byte-verbatim, in the pre-migration sheet's own order. `react-flow.css` holds the React Flow overrides **unlayered**, imported from `RunExecution.tsx` on the statement immediately after `@xyflow/react/dist/style.css`. **New markup styles go in `@layer components` in `theme.css`. A rule goes in `react-flow.css` only if it competes with an unlayered xyflow rule** — that means it styles an element inside `<ReactFlow>` AND sets a property xyflow sets on the same element. Rendering inside the canvas is not by itself enough: `.iteration-badge`, `.gate-waiting-hint` and the `.pill-*` states all render inside a node and all are layered, because xyflow says nothing about them. A selector must never live in both files and a rule is deleted and re-added in ONE edit; `e2e/cssPort.test.ts` fails on either, asserts the partition in both directions, and asserts the import adjacency, and `e2e/style-diff.spec.ts` compares computed styles against the merge-base build across 17 routes x 5 widths and asserts the built cascade order.
...
- **F2 — WCAG AA contrast** on `pill-succeeded` and dim monospace metadata. The three `--ink-*` tokens every screen inherits now live in the `:root` rule inside `theme.css`'s `@layer components` (they moved there in M9 PR5a and keep their names; unifying them with `--color-ink-*` is a value substitution and was deferred out of PR5c to the token-substitution follow-up); `.pill` and the `.pill-*` state rules are there too, moved by PR5c. Fixing a token fixes every page at once; shipping new pages on the current values spreads the finding. Any change to those values is a rendering change and must be run past `e2e/style-diff.spec.ts`, which will report it.
